...
3. Select `com.redjohn.mobile-avatar-studio-0.1.4.tgz`.
...
Then add **RedJohn Mobile Avatar Studio** to the project. The listing contains the current `0.1.4` release and preserves older versions for existing projects.
...
- **Unity reports invalid package dependencies:** verify that the tarball contains a top-level `package/` directory and that the package is installed from the current `0.1.4` archive.
