...
The first public release was `0.1.0`; the current maintenance release is `0.1.3`. It is open source under the MIT license.

## Quick install

For a normal user, download `com.redjohn.mobile-avatar-studio-0.1.3.tgz` from the [v0.1.3 release](https://github.com/JustYaz/redjohn-mobile-avatar-studio/releases/tag/v0.1.3) and add it through Unity Package Manager. For the latest development version, use this Git URL:
...
3. Select `com.redjohn.mobile-avatar-studio-0.1.3.tgz`.
...
The Git URL follows the `main` branch. Use the tagged release tarball when you need a reproducible version.
...
Then add **RedJohn Mobile Avatar Studio** to the project. The listing contains the current `0.1.3` release and preserves older versions for existing projects.
...
- **Unity reports invalid package dependencies:** verify that the tarball contains a top-level `package/` directory and that the package is installed from the current `0.1.3` archive.
